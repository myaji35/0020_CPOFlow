...
| 메뉴 | 초기 | 현재 | 변동 | 핵심 미흡사항 | Wave |
|------|------|------|------|-------------|------|
| 대시보드 | 95% | **95%** | — | 실시간 갱신 없음 | — |
| 받은편지함 | 90% | **90%** | — | API 크레딧 소진 시 AI fallback UX | W3 |
| 칸반 | 85% | **88%** | +3 | 커스텀 보드 폼/이동/탭 분기 완성. 미세 UX 잔여 | W3 |
| 발주 목록 | 80% | **85%** | +5 | 보드별 폼+@담당자+URL첨부+GDrive. PDF 레이아웃 기본 | W2 |
| 일정 | 75% | **90%** | +15 | 주간 뷰+오늘 강조+상태 색상+드로어 연동 완성 | ✅ W1-D |
| 팀 | 80% | **95%** | +15 | KPI 6종+성과 bar chart+담당자 분포 완성 | ✅ W2-A |
| 발주처 | 85% | **85%** | — | 리스크 자동 계산 정확도 | W3 |
| 거래처 | 85% | **85%** | — | eCount API 동기화 미작동 | W3 |
| 외부 담당자 | 80% | **95%** | +15 | KPI+소속 필터+검색 강화+이메일 이력+서명 추출 개선 | ✅ W2-B |
| 현장 | 80% | **95%** | +15 | KPI+주문목록+예산 카드+검색 강화+활성 필터 | ✅ W2-C |
| 직원 관리 | 85% | **85%** | — | 갱신 워크플로우 자동화 없음 | W3 |
| 조직도 | 75% | **90%** | +15 | 트리 시각화+접기/펼치기+검색+통계 카드 완성 | ✅ W1-C |
| 경영 리포트 | 70% | **90%** | +20 | PDF 내보내기+보드 필터+차트 클릭 인터랙션 완성 | ✅ W1-B |
| eCount | 60% | **80%** | +20 | 거래내역 실질 뷰+KPI+필터+정렬 완성. 거래처 API 미확보 잔여 | ✅ W1-A |
| 설정 | 85% | **90%** | +5 | 보드CRUD+블럭관리+카드상태 편집수정+breadcrumb | W3 |
| 공통 | 80% | **90%** | +10 | 검색 빈 상태+알림 상대 시간+UI 개선 | ✅ W2-D |
| **전체 평균** | **81%** | **91%** | **+10** | Wave 1+2 완료. Wave 3→95% 진행 예정 | |
