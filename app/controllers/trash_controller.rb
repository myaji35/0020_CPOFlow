class TrashController < ApplicationController
  ...
  def index
    # LAB sandbox는 trash에서도 제외 (운영 영역 격리)
    base = Order.archived.not_sandbox.order(archived_at: :desc)
  ...
  def empty
    ...
    scope = Order.archived.not_sandbox
    count = scope.count
    scope.destroy_all
  ...
  def set_order
    base = Order.archived.not_sandbox
