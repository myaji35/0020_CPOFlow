...
async function performLogin(page) {
  log("Login required, performing 2-step login...");

  // Step 1: Username
  try {
    const userField = await page.waitForSelector(
      'input[name="UserName"], input[type="email"], #username, input[name="username"]',
  ...
  await saveScreenshot(page, "01_username_filled");

  // Click submit after username
  const submitBtn = await page.$('button[type="submit"], input[type="submit"], #nextBtn, .submitBtn');
  if (submitBtn) {
    // Use Promise.allSettled to handle navigation that may or may not occur
    await Promise.allSettled([
      page.waitForNavigation({ timeout: 15000 }).catch(() => {}),
      submitBtn.click(),
    ]);
    log("Username submitted, waiting for password step...");
    await page.waitForTimeout(3000);
  }

  await saveScreenshot(page, "02_after_username_submit");
  ...
  try {
    ...
    // Ariba sometimes hides password with displayNone class
    log("Password field not visible, trying force-show...");
    try {
      await page.evaluate(() => {
      ...
      if (passField) {
        await passField.fill(PASSWORD);
        passwordFilled = true;
        log("Password entered (force-shown)");
      }
    } catch (e2) {
      log(`Password field not found even after force-show: ${e2.message}`);
    }
  }

  if (!passwordFilled) {
    log("ERROR: Could not fill password field");
    await saveScreenshot(page, "03_password_failed");
    return false;
  }

  await saveScreenshot(page, "03_password_filled");

  // Final login submit - use Promise.allSettled for the navigation
  const loginBtn = await page.$('button[type="submit"], input[type="submit"], #nextBtn, .submitBtn');
  if (loginBtn) {
    await Promise.allSettled([
  ...
  await waitForStableUrl(page, 30000);
