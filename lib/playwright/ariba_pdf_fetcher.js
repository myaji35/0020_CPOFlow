...
async function findAndDownloadPdfs(page) {
  ...
  // Strategy 1: Direct attachment links (PDF + Office docx/xlsx + 압축 + 일반 download)
  // 확장자 → MIME 매핑 — Ariba 가 Content-Disposition 으로 정확한 파일명 주므로 추론 안전
  const EXT_TO_CT = {
    pdf:  "application/pdf",
  ...
  const SUPPORTED_EXTS = Object.keys(EXT_TO_CT);
  const inferContentType = (filename) => {
    const m = (filename || "").toLowerCase().match(/\.([a-z0-9]+)$/);
    return (m && EXT_TO_CT[m[1]]) || "application/octet-stream";
  };

  let attachmentLinks = [];
  try {
    const extSelectors = SUPPORTED_EXTS.flatMap((ext) => [
      `a[href*=".${ext}"]`,
      `a[title*=".${ext}"]`,
    ]);
    ...
    const combinedSelector = [...extSelectors, ...genericSelectors].join(", ");

    attachmentLinks = await page.$$eval(
      combinedSelector,
      (els) =>
        els
    ...
    log(`Attachment link search failed: ${e.message}`);
  }
  const uniqueHrefs = [...new Set(attachmentLinks.map((l) => l.href))];
  log(`Found ${uniqueHrefs.length} potential attachment links (PDF/Office/zip 등 다중 형식)`);

  for (const linkUrl of uniqueHrefs) {
    try {
      ...
      // Content-Disposition 으로 받은 정확한 파일명 우선
      const filename = download.suggestedFilename() || `ariba_doc_${Date.now()}`;
      const savePath = path.join(OUTPUT_DIR, filename);
      await download.saveAs(savePath);

      const contentType = inferContentType(filename);
      log(`Downloaded: ${filename} (${contentType})`);
      results.push({ filename, path: savePath, content_type: contentType });
    } catch (dlErr) {
      log(`Download failed for ${linkUrl}: ${dlErr.message}`);
    }
  }
    ...
    try {
      ...
      for (const btn of printBtns) {
        ...
        try {
          ...
          await download.saveAs(savePath);
          const contentType = inferContentType(filename);
          log(`Downloaded via button: ${filename} (${contentType})`);
          results.push({ filename, path: savePath, content_type: contentType });
        } catch (e) {
  ...
  if (results.length === 0) {
    ...
    try {
      ...
      results.push({ filename, path: savePath, content_type: "application/pdf" });
    } catch (e) {
    ...
    if (results.length === 0 && pageInfo.iframeCount > 0) {
      ...
      for (const frame of frames) {
        ...
        try {
          ...
          const frameAttachmentLinks = await frame.$$eval(
            'a[href*=".pdf"], a[href*=".docx"], a[href*=".doc"], a[href*=".xlsx"], a[href*=".xls"], a[href*=".pptx"], a[href*=".zip"], a[href*="download"], a[href*="attachment"], a[href*="getDocument"], a[href*="documentDownload"]',
            (els) => els.map((el) => el.href).filter(Boolean)
          );
          if (frameAttachmentLinks.length > 0) {
            log(`Found ${frameAttachmentLinks.length} attachment links in iframe (다중 형식)`);
            const inferIframeCT = (fn) => {
              const m = (fn || "").toLowerCase().match(/\.([a-z0-9]+)$/);
              const map = { pdf: "application/pdf", docx: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", doc: "application/msword", xlsx: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", xls: "application/vnd.ms-excel", pptx: "application/vnd.openxmlformats-officedocument.presentationml.presentation", zip: "application/zip", csv: "text/csv", txt: "text/plain" };
              return (m && map[m[1]]) || "application/octet-stream";
            };
            for (const linkUrl of frameAttachmentLinks) {
              try {
                const [download] = await Promise.all([
                  page.waitForEvent("download", { timeout: 15000 }),
                  frame.evaluate((url) => {
                ...
                const filename = download.suggestedFilename() || `ariba_iframe_${Date.now()}`;
                const savePath = path.join(OUTPUT_DIR, filename);
                await download.saveAs(savePath);
                const contentType = inferIframeCT(filename);
                log(`Downloaded from iframe: ${filename} (${contentType})`);
                results.push({ filename, path: savePath, content_type: contentType });
              } catch (e) {
